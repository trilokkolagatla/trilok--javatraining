...
public class Tversky {
	...
	public double getCoefficient(String s1, String s2) {
		...
		double p = Math.abs((first.size() - second.size()));
		double t = Math.abs((second.size() - first.size()));
		double alpha =1.0, beta =1.0;
		double f = d / (d + alpha * p + beta * t);
		return f;
	}

	public static void main(String[] args) {
		Tversky tver = new Tversky();
		String s1 = "fasdd";
		String s2 = "udd";
		double f = tver.getCoefficient(s1, s2);
